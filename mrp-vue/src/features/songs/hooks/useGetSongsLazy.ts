...
export type SongOption = { id: number; name: string; year?: number | null };

const PAGE_SIZE = 10;
const DEBOUNCE_MS = 300;

export const useGetSongsLazy = () => {
  const items = ref<SongOption[]>([]);
  ...
  const fetchPage = async (q: string, pageNum: number) => {
    ...
    try {
      ...
      const incoming: SongOption[] = data.content.map((s) => ({ id: s.id, name: s.name, year: s.year }));
      page = pageNum;
