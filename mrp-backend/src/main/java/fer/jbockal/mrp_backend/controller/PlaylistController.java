...
public class PlaylistController {
    ...
    public ResponseEntity<List<PlaylistResponseDto>> mine(
        ...
        return ResponseEntity.ok(playlistService.listMine(principal, page, size));
    }

    @GetMapping("/public")
    public ResponseEntity<List<PlaylistResponseDto>> listPublic(
            @RequestParam(defaultValue = "0") int page,
            @RequestParam(defaultValue = "20") int size
    ) {
        return ResponseEntity.ok(playlistService.listPublic(page, size));
    }

    @GetMapping("/admin")
    public ResponseEntity<List<PlaylistResponseDto>> adminList(
            @AuthenticationPrincipal Object principal,
            @RequestParam(defaultValue = "0") int page,
            @RequestParam(defaultValue = "20") int size
    ) {
        return ResponseEntity.ok(playlistService.listAllAsAdmin(principal, page, size));
    }
